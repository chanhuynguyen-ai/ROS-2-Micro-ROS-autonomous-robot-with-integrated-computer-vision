...
class PurPersuitMainlaneFollowing(Node):
    """
    Bộ điều khiển Pure Pursuit cho xe bám main-lane.
    ...
    Input JSON chính:
      epsilon_x_mm       : sai số ngang, mm. Dương nếu target ở bên phải xe.
      epsilon_y_mm       : khoảng cách nhìn trước Ld, mm.
      theta_rad          : sai số góc, rad.
    ...
    Công thức:
      ...
      Nhưng hệ robot:
        ...
        ROS angular.z > 0: quay trái.
      Vì vậy node dùng:
        omega = -2 * v * e_x / L_d^2
    """

    def __init__(self):
        super().__init__("pur_persuit_mainlane_following")

        self.declare_parameter("control_error_topic", "/avs/control_error")
        self.declare_parameter("cmd_vel_topic", "/cmd_vel")
        self.declare_parameter("debug_topic", "/avs/pur_persuit_mainlane_debug")
        ...
        # Phạt độ cong. Với dữ liệu thực tế của xe, để thấp hơn guide 200-500
        # vì curvature_inv_mm của AVS thường đã khá lớn sau khi đổi sang m^-1.
        self.declare_parameter("k_c", 0.20)

        # Giới hạn yaw rate.
        self.declare_parameter("omega_max", 0.35)

        # Hình học xe: dùng để đổi omega thành chênh lệch tốc độ hai bên.
        ...
        self.get_logger().info("pur_persuit_mainlane_following started")
        self.get_logger().info(f"Subscribe: {self.control_error_topic}")
        self.get_logger().info(f"Publish:   {self.cmd_vel_topic}")
        self.get_logger().info("Control law: Pure Pursuit from /avs/control_error")
    ...
    def control_loop(self):
        ...
        # 1. Đọc và đổi đơn vị theo guide.
        ...
        # 2. Lọc e_x và theta để xe không giật.
        ...
        # 4. Tính omega theo Pure Pursuit:
        #    gamma = 2*e_x/L_d^2
        #    omega = v*gamma
        # Dấu âm vì epsilon_x dương là bên phải, ROS omega âm mới rẽ phải.
        ...
        # 5. Đổi omega thành chênh lệch vận tốc hai bên:
        ...
        self.publish_debug(
            "tracking_pure_pursuit_mainlane",
            {
                "e_x_m_raw": e_x,
                "L_d_m": L_d,
...
def main(args=None):
    ...
    node = PurPersuitMainlaneFollowing()
