...
class MainFollowingPD(Node):
    """
    Bộ điều khiển PD cho xe bám main-lane.
    ...
    Publish:
      ...
      /avs/main_following_pd_debug [std_msgs/String]
    ...
    def __init__(self):
        super().__init__("main_following_pd")

        self.declare_parameter("control_error_topic", "/avs/control_error")
        self.declare_parameter("cmd_vel_topic", "/cmd_vel")
        self.declare_parameter("debug_topic", "/avs/main_following_pd_debug")
        ...
        # Giữ lại parameter k_pp như code cũ, nhưng node PD-only không dùng để tính omega.
        ...
        self.get_logger().info("main_following_pd started")
        self.get_logger().info(f"Subscribe: {self.control_error_topic}")
        self.get_logger().info(f"Publish:   {self.cmd_vel_topic}")
        self.get_logger().info("Control law: PD only from /avs/control_error")
        self.get_logger().info("Pure Pursuit removed: omega_target = omega_pd")
    ...
    def control_loop(self):
        ...
        # 5. PD only:
        #    Không còn Pure Pursuit:
        #    Không tính gamma.
        #    Không tính omega_pp từ e_x / L_d^2.
        #
        ...
        # 6. Đổi omega thành chênh lệch vận tốc hai bên:
