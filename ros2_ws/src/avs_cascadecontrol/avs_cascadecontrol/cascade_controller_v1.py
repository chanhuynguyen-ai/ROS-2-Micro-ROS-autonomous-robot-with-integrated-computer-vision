...
class CascadeControllerV1(Node):
    def __init__(self):
        super().__init__("cascade_controller_v1")
        ...
        self.declare_parameter("ref_topic", "/avs/cascade_controller_v1_ref")
        self.declare_parameter("state_topic", "/avs/cascade_controller_v1_state")
        ...
        self.declare_parameter("blind_hold_s", 0.55)
        self.declare_parameter("startup_straight_s", 0.30)
        self.declare_parameter("startup_v", 0.050)
        ...
        self.declare_parameter("target_distance_per_frame_m", 0.038)

        self.declare_parameter("v_max", 0.140)
        self.declare_parameter("v_center", 0.120)
        self.declare_parameter("v_min", 0.045)
        self.declare_parameter("v_curve_max", 0.070)
        self.declare_parameter("v_lane_change_max", 0.060)
        self.declare_parameter("v_recover_max", 0.085)
        self.declare_parameter("v_large_error", 0.040)
        self.declare_parameter("v_blind", 0.038)
        self.declare_parameter("v_jump_hold", 0.035)
        ...
        self.declare_parameter("error_filter_tau_s", 0.72)
        self.declare_parameter("derivative_filter_tau_s", 1.35)

        self.declare_parameter("x_deadband_m", 0.022)
        self.declare_parameter("theta_deadband_rad", 0.040)

        self.declare_parameter("center_x_m", 0.055)
        self.declare_parameter("center_theta_rad", 0.105)

        self.declare_parameter("near_x_m", 0.095)
        self.declare_parameter("near_theta_rad", 0.200)

        self.declare_parameter("large_error_x_m", 0.190)
        self.declare_parameter("large_error_theta_rad", 0.620)

        self.declare_parameter("curve_enter_x_m", 0.145)
        self.declare_parameter("curve_enter_theta_rad", 0.330)
        self.declare_parameter("curve_enter_kappa", 1.65)
        self.declare_parameter("curve_confirm_frames", 4)
        self.declare_parameter("curve_release_frames", 8)

        self.declare_parameter("max_abs_x_m", 0.55)
        self.declare_parameter("max_abs_theta_rad", 1.45)
        self.declare_parameter("min_valid_lookahead_m", 0.16)

        self.declare_parameter("jump_guard_enable", True)
        self.declare_parameter("jump_x_m", 0.280)
        self.declare_parameter("jump_theta_rad", 0.720)
        self.declare_parameter("jump_hold_s", 0.20)

        self.declare_parameter("soft_replan_enable", True)
        self.declare_parameter("soft_replan_step_x_m", 0.045)
        self.declare_parameter("soft_replan_step_theta_rad", 0.120)
        self.declare_parameter("soft_replan_hold_s", 0.55)
        self.declare_parameter("soft_replan_v_max", 0.048)
        self.declare_parameter("soft_replan_omega_max", 0.145)

        self.declare_parameter("k_pp_near", 0.04)
        self.declare_parameter("k_lat_near", 0.10)
        self.declare_parameter("k_theta_near", 0.05)

        self.declare_parameter("k_pp_mid", 0.36)
        self.declare_parameter("k_lat_mid", 0.34)
        self.declare_parameter("k_theta_mid", 0.13)

        self.declare_parameter("k_pp_curve", 0.78)
        self.declare_parameter("k_lat_curve", 0.58)
        self.declare_parameter("k_theta_curve", 0.22)

        self.declare_parameter("k_pp_large", 0.08)
        self.declare_parameter("k_lat_large", 0.24)
        self.declare_parameter("k_theta_large", 0.10)

        self.declare_parameter("kd_lat", 0.002)
        self.declare_parameter("kd_theta", 0.003)

        self.declare_parameter("k_slow_x", 1.10)
        self.declare_parameter("k_slow_theta", 0.80)
        self.declare_parameter("k_slow_kappa", 0.055)

        self.declare_parameter("omega_center_max", 0.000)
        self.declare_parameter("omega_near_max", 0.015)
        self.declare_parameter("omega_mid_max", 0.135)
        self.declare_parameter("omega_curve_max", 0.360)
        self.declare_parameter("omega_lane_change_max", 0.240)
        self.declare_parameter("omega_large_error_max", 0.160)
        self.declare_parameter("omega_abs_max", 0.420)
        self.declare_parameter("omega_deadband", 0.012)

        self.declare_parameter("v_rate_up", 0.18)
        self.declare_parameter("v_rate_down", 0.38)
        self.declare_parameter("omega_rate_center", 0.24)
        self.declare_parameter("omega_rate_mid", 0.34)
        self.declare_parameter("omega_rate_curve", 0.50)
        self.declare_parameter("omega_rate_reverse", 0.80)

        self.declare_parameter("ld_min_m", 0.340)
        self.declare_parameter("ld_max_m", 0.85)
        self.declare_parameter("default_lookahead_m", 0.48)
        self.declare_parameter("max_abs_kappa", 2.0)

        self.declare_parameter("v_max_cmd", 0.14)
        self.declare_parameter("omega_max_cmd", 0.60)

        self.declare_parameter("wheel_speed_rate", 0.105)
        self.declare_parameter("wheel_speed_decel_rate", 0.260)

        self.declare_parameter("same_direction_inner_fraction", 0.62)
        self.declare_parameter("same_direction_min_inner_mps", 0.030)

        self.declare_parameter("enable_soft_counter_turn", False)
        self.declare_parameter("counter_start_omega", 0.45)
        self.declare_parameter("counter_full_omega", 0.80)
        self.declare_parameter("counter_outer_min_mps", 0.040)
        self.declare_parameter("counter_outer_max_mps", 0.060)
        self.declare_parameter("counter_reverse_max_mps", 0.006)

        self.declare_parameter("inner_omega_deadband", 0.024)

        self.declare_parameter("enable_calibration", True)
        self.declare_parameter("linear_cmd_scale", 1.245)
        self.declare_parameter("angular_cmd_scale", 1.08)
        ...
        self.declare_parameter("avoidance_direction", "left")
        self.declare_parameter("avoid_trigger_distance", 0.55)
        self.declare_parameter("avoid_clear_distance", 0.82)
        self.declare_parameter("avoid_clear_hold_s", 0.85)
        self.declare_parameter("avoid_min_duration_s", 1.80)
        self.declare_parameter("avoid_lateral_offset_m", 0.220)
        self.declare_parameter("avoid_offset_rate_mps", 0.085)
        self.declare_parameter("avoidance_v_max", 0.060)
        self.declare_parameter("avoidance_omega_max", 0.220)
        self.declare_parameter("avoidance_min_omega", 0.045)
        self.declare_parameter("avoid_lidar_min_ratio", 0.35)
        self.declare_parameter("avoid_danger_distance", 0.30)
        self.declare_parameter("avoid_reverse_speed", -0.035)
        self.declare_parameter("avoid_reverse_omega", 0.0)
        self.declare_parameter("allow_reverse_cmd", True)
        self.declare_parameter("avoid_reverse_duration_s", 0.55)
        self.declare_parameter("avoid_reverse_cooldown_s", 2.20)
        self.declare_parameter("avoid_force_shift_after_reverse_s", 1.20)
        self.declare_parameter("avoid_return_max_front_m", 10.0)
        self.declare_parameter("avoid_return_error_m", 0.070)
        ...
        self.avoid_reverse_until = -1.0
        self.avoid_reverse_cooldown_until = -1.0
        self.avoid_force_shift_until = -1.0
        self.avoid_last_mode_change_time = -1.0
        ...
        self.get_logger().info("cascade_controller_v1_4_obstacle_state_machine started")
        self.get_logger().info(f"control_error: {self.control_error_topic}")
        self.get_logger().info(f"lane_state: {self.lane_state_topic}")
        self.get_logger().info(f"cmd_vel: {self.cmd_vel_topic}")
        self.get_logger().info(f"state: {self.state_topic}")
    ...
    def signal_stop_handler(self, signum, frame):
        self.get_logger().warn(f"Signal {signum}. Sending stop burst.")
    ...
    def runtime_enable_cb(self, msg):
        ...
        self.get_logger().warn(f"runtime_enable={self.runtime_enable}")
        if not self.runtime_enable and not self.pbool("enable_cmd"):
            self.publish_stop_burst()

    def emergency_stop_cb(self, msg):
        self.emergency_stop = bool(msg.data)
        self.get_logger().error(f"emergency_stop={self.emergency_stop}")
    ...
    def extract_control_error(self, data):
        ...
        kappa = clamp(
            curvature_inv_mm * 1000.0,
            -self.pfloat("max_abs_kappa"),
            self.pfloat("max_abs_kappa"),
        )
        ...
        if lane_state in ["LOST", "INVALID", "NO_LANE", "NONE"]:
            return False, "lane_state_invalid", e_lat, e_theta, lookahead, kappa, confidence, lane_state, lat_source, theta_source, lookahead_source

        if abs(e_lat) > self.pfloat("max_abs_x_m"):
            return False, "e_lat_outlier", e_lat, e_theta, lookahead, kappa, confidence, lane_state, lat_source, theta_source, lookahead_source

        if abs(e_theta) > self.pfloat("max_abs_theta_rad"):
            return False, "theta_outlier", e_lat, e_theta, lookahead, kappa, confidence, lane_state, lat_source, theta_source, lookahead_source

        if lookahead < self.pfloat("min_valid_lookahead_m"):
            return False, "lookahead_too_near", e_lat, e_theta, lookahead, kappa, confidence, lane_state, lat_source, theta_source, lookahead_source

        valid = raw_valid and lane_valid
        if lane_state == "FOLLOW_MAIN":
            valid = True

        return valid, "ok" if valid else "invalid_flags", e_lat, e_theta, lookahead, kappa, confidence, lane_state, lat_source, theta_source, lookahead_source
    ...
    def control_error_cb(self, msg):
        ...
        valid, reason, e, theta, ld, kappa, conf, lane_state, lat_src, theta_src, ld_src = self.extract_control_error(data)
        ...
        self.raw_lat_source = lat_src
        self.raw_theta_source = theta_src
        self.raw_lookahead_source = ld_src
        ...
        if jump:
            ...
            if self.pbool("soft_replan_enable") and self.last_accepted_e is not None and self.last_accepted_theta is not None:
                ...
                de = clamp(e - self.last_accepted_e, -step_x, step_x)
                dth = clamp(theta - self.last_accepted_theta, -step_th, step_th)

                e = self.last_accepted_e + de
                theta = self.last_accepted_theta + dth
    ...
    def update_curve_state(self, e, theta, kappa, intent_hint):
        intent_curve = intent_hint in ["TURN_LEFT", "TURN_RIGHT", "LANE_CHANGE"]

        candidate = (
            intent_curve
            or abs(e) >= self.pfloat("curve_enter_x_m")
    ...
    def compute_outer(self, e, theta):
        ...
        curve = self.update_curve_state(e, theta, kappa, intent)

        center = (
            abs(self.e_f) <= self.pfloat("center_x_m")
            and abs(self.theta_f) <= self.pfloat("center_theta_rad")
            and not self.avoidance_mode.startswith("avoid_")
        )
        ...
        lane_change = intent == "LANE_CHANGE"
        ...
        theta_term_sign = self.pfloat("theta_control_sign")
        ...
        if center:
            ...
            mode = "lane_change_committed"
            v_des = clamp(v_base * slow_factor, self.pfloat("v_min"), self.pfloat("v_lane_change_max"))
            omega_limit = self.pfloat("omega_lane_change_max")
            k_pp = self.pfloat("k_pp_curve")
            k_lat = 0.85 * self.pfloat("k_lat_curve")
            k_theta = 0.85 * self.pfloat("k_theta_curve")
            omega_raw = (
                k_pp * v_des * curvature_from_error
                - k_lat * e
                + theta_term_sign * k_theta * theta
    ...
    def avoidance_lateral_sign(self):
        direction = self.pstr("avoidance_direction").strip().lower()
        if direction == "right":
            return 1.0
        return -1.0
    ...
    def update_obstacle_avoidance(self, now, dt):
        ...
        danger = self.pfloat("avoid_danger_distance")
        trigger = self.pfloat("avoid_trigger_distance")
        clear = self.pfloat("avoid_clear_distance")

        front_valid = math.isfinite(self.front_min)
        front = self.front_min if front_valid else float("inf")

        def set_mode(new_mode):
            if self.avoidance_mode != new_mode:
                self.avoidance_mode = new_mode
                self.avoid_last_mode_change_time = now

        # Emergency is a real stop. Do not try to maneuver if too close.
        if front_valid and front < emergency:
            set_mode("avoid_emergency_wait")
            self.avoidance_offset_target = 0.0
            self.avoid_reverse_until = -1.0

        # Reverse phase is time-based. It cannot restart every cycle.
        elif self.avoidance_mode == "avoid_reverse":
            self.avoidance_offset_target = 0.0

            if now >= self.avoid_reverse_until:
                set_mode("avoid_shift_out")
                self.avoid_reverse_cooldown_until = now + self.pfloat("avoid_reverse_cooldown_s")
                self.avoid_force_shift_until = now + self.pfloat("avoid_force_shift_after_reverse_s")
                self.avoidance_start_time = now
                self.avoidance_clear_start_time = -1.0
                self.avoidance_offset_target = (
                    self.avoidance_lateral_sign()
                    * abs(self.pfloat("avoid_lateral_offset_m"))
                )

        elif self.avoidance_mode in ["lane_follow", "disabled", "avoid_emergency_wait"]:
            self.avoidance_offset_target = 0.0

            if front_valid and front < danger and now >= self.avoid_reverse_cooldown_until and self.pbool("allow_reverse_cmd"):
                set_mode("avoid_reverse")
                self.avoid_reverse_until = now + self.pfloat("avoid_reverse_duration_s")
                self.avoidance_start_time = now
                self.avoidance_clear_start_time = -1.0

            elif front_valid and front < trigger:
                set_mode("avoid_shift_out")
                self.avoidance_start_time = now
                self.avoidance_clear_start_time = -1.0
                self.avoidance_offset_target = (
            ...
            # After reverse, force shifting right even if lidar still sees the object.
            if abs(self.avoidance_offset_current - self.avoidance_offset_target) < 0.030:
                set_mode("avoid_bypass")
            elif now >= self.avoid_force_shift_until and self.avoid_force_shift_until > 0.0:
                set_mode("avoid_bypass")
            ...
            # Do not go back to reverse immediately while returning.
            if front_valid and front < danger and now >= self.avoid_reverse_cooldown_until and self.pbool("allow_reverse_cmd"):
                set_mode("avoid_reverse")
                self.avoid_reverse_until = now + self.pfloat("avoid_reverse_duration_s")
                self.avoidance_start_time = now
            ...
            elif abs(self.avoidance_offset_current) < 0.012 and abs(self.e_f) < self.pfloat("avoid_return_error_m"):
                set_mode("lane_follow")
                self.avoidance_start_time = -1.0
                self.avoidance_clear_start_time = -1.0
                self.avoid_force_shift_until = -1.0
        ...
        active = self.avoidance_mode in ["avoid_shift_out", "avoid_bypass", "avoid_return"]
        reverse_active = self.avoidance_mode == "avoid_reverse" and now < self.avoid_reverse_until
    ...
    def lidar_scale(self):
        ...
        avoid_active = (
            self.pbool("enable_obstacle_avoidance")
            and self.avoidance_mode in ["avoid_shift_out", "avoid_bypass", "avoid_return"]
        )

        if avoid_active and self.front_min < stop:
            ratio = max(0.20, self.pfloat("avoid_lidar_min_ratio"))
            return ratio, False, "lidar_avoid_close"
        ...
        if avoid_active and self.front_min < slow:
            ratio = clamp(
                (self.front_min - stop) / max(1e-6, slow - stop),
                self.pfloat("avoid_lidar_min_ratio"),
                1.0,
            )
    ...
    def mix_wheels(self, v_ref, omega_ref, dt):
        ...
        v_ref = clamp(v_ref, 0.0, abs(self.pfloat("v_max_cmd")))
        ...
        if v_ref <= 0.0:
            ...
            abs_w = abs(omega_ref)
            sign_w = 1.0 if omega_ref > 0.0 else -1.0 if omega_ref < 0.0 else 0.0

            counter_enable = self.pbool("enable_soft_counter_turn")
            counter_start = abs(self.pfloat("counter_start_omega"))
            counter_full = max(abs(self.pfloat("counter_full_omega")), counter_start + 1e-3)

            if counter_enable and abs_w >= counter_start and sign_w != 0.0:
                strength = clamp((abs_w - counter_start) / (counter_full - counter_start), 0.0, 1.0)

                outer_min = self.pfloat("counter_outer_min_mps")
                outer_max = self.pfloat("counter_outer_max_mps")
                reverse_max = abs(self.pfloat("counter_reverse_max_mps"))

                outer_speed = clamp(v_ref, outer_min, outer_max)
                inner_reverse = reverse_max * strength

                if sign_w > 0.0:
                    v_left_des = -inner_reverse
                    v_right_des = outer_speed
                else:
                    v_left_des = outer_speed
                    v_right_des = -inner_reverse

                mix_mode = "soft_counter_turn"
            else:
                ...
                inner_min_abs = abs(self.pfloat("same_direction_min_inner_mps"))

                if omega_ref > 0.0:
                    outer = max(v_right_des, inner_min_abs)
                    inner_min = max(inner_min_abs, outer * inner_frac)
                    v_left_des = max(v_left_des, inner_min)
                    v_right_des = outer
                    mix_mode = "same_direction_left_turn"
                elif omega_ref < 0.0:
                    outer = max(v_left_des, inner_min_abs)
                    inner_min = max(inner_min_abs, outer * inner_frac)
                    v_right_des = max(v_right_des, inner_min)
                    v_left_des = outer
        ...
        if abs(v_left_des) >= abs(self.v_left_cmd_prev):
            left_rate = abs(self.pfloat("wheel_speed_rate"))
        else:
            left_rate = abs(self.pfloat("wheel_speed_decel_rate"))

        if abs(v_right_des) >= abs(self.v_right_cmd_prev):
            right_rate = abs(self.pfloat("wheel_speed_rate"))
        else:
            right_rate = abs(self.pfloat("wheel_speed_decel_rate"))
        ...
        if self.pbool("enable_calibration"):
            linear_scale = max(1e-6, self.pfloat("linear_cmd_scale"))
            angular_scale = max(1e-6, self.pfloat("angular_cmd_scale"))
    ...
    def control_loop(self):
        ...
        jump_hold = now < self.jump_hold_until
        ...
        reverse_active = bool(avoid_info.get("reverse_active", False))
        ...
        if reverse_active:
            ...
            mode = "avoid_reverse"
            v_des = self.pfloat("avoid_reverse_speed")
            omega_des = self.pfloat("avoid_reverse_omega")

            v_rate = max(self.pfloat("v_rate_down"), 0.24)
            omega_rate = self.pfloat("omega_rate_reverse")
            ...
            if self.pbool("enable_calibration"):
                v_cmd = self.v_ref / max(1e-6, self.pfloat("linear_cmd_scale"))
                omega_cmd = self.omega_ref / max(1e-6, self.pfloat("angular_cmd_scale"))
            else:
                v_cmd = self.v_ref
            ...
            cmd_published, conflict, publishers, publish_reason = self.maybe_publish_cmd(v_cmd, omega_cmd)
            ...
            self.publish_state({
                "node": "cascade_controller_v1",
                "version": "cascade_controller_v1_4_obstacle_state_machine",
                "time": now,
                ...
                "avoidance_mode": "avoid_reverse",
                "avoidance_active": True,
                "avoidance_reverse": True,
                "avoidance_offset_m": self.avoidance_offset_current,
                "avoidance_target_offset_m": 0.0,
                ...
                "curve_confirmed": self.curve_confirmed,
                ...
                "omega_des": omega_des,
                "omega_raw": omega_des,
                "omega_limit": abs(omega_des),
                "v_ref": self.v_ref,
                "omega_ref": self.omega_ref,

                "v_left_des": self.v_ref,
                "v_right_des": self.v_ref,
                "v_left_cmd": self.v_cmd,
                "v_right_cmd": self.v_cmd,
                "v_cmd": self.v_cmd,
        ...
        if avoid_info["active"] and mode not in ["control_error_timeout"]:
        ...
        if mode in ["center_cruise", "near_center_cascade", "startup_straight", "blind_straight_hold", "jump_guard_straight"]:
            omega_rate = self.pfloat("omega_rate_center")
        elif mode in ["curve_committed_fast_response", "lane_change_committed", "avoid_shift_out", "avoid_bypass", "avoid_return"]:
