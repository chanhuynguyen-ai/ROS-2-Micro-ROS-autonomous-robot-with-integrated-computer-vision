...
class HybridController(Node):
    ...
    VERSION = "hybrid_bs_cascade_pd_v1_1"
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "ref_topic",
            "/avs/hybrid_controller_ref"
        )

        self.declare_parameter(
            "state_topic",
            "/avs/hybrid_controller_state"
        )

        self.declare_parameter(
            "runtime_enable_topic",
            "/avs/cascade_v3_enable_cmd"
        )
        ...
        # ============================================================
        # OUTER CASCADE PD + BACKSTEPPING HYBRID
        #
        # CASCADE PD formula:
        #   omega_pd = sign * (
        #       Kp_lat*e_lat + Kd_lat*de_lat +
        #       Kp_theta*e_theta + Kd_theta*de_theta
        #   )
        #
        # BACKSTEPPING formula:
        #   theta_v  = atan(lambda_y * e_lat)
        #   e_bs     = e_theta + theta_v
        #   omega_bs = -(k_y*e_lat + k_dy*de_lat
        #              + k_theta*e_bs + k_dtheta*de_bs)
        #
        # BLEND:
        #   omega = (1-bs_weight)*omega_pd + bs_weight*omega_bs
        # ============================================================
        ...
        # --- Backstepping gains ---
        self.declare_parameter(
            "bs_weight",        # 0.0=pure cascade, 1.0=pure backstepping
            0.55
        )

        self.declare_parameter(
            "lambda_y",         # slope of virtual heading curve
            0.55
        )

        self.declare_parameter(
            "k_y",              # lateral P in backstepping
            0.55
        )

        self.declare_parameter(
            "k_dy_bs",          # lateral D in backstepping
            0.45
        )

        self.declare_parameter(
            "k_theta",          # heading P in backstepping
            0.42
        )

        self.declare_parameter(
            "k_dtheta_bs",      # heading D in backstepping
            0.30
        )
        ...
        # Tăng trọng số kappa để phát hiện cua sớm từ xa (chậm lại trước khi vào rẽ)
        self.declare_parameter(
            "curve_kappa_weight",
            0.45
        )
    ...
    def extract_error(
        ...
        raw_la_mm = data.get("lookahead_d_mm")
        if raw_la_mm is not None:
            lookahead = finite_float(raw_la_mm, 0.0) / 1000.0
        else:
    ...
    def compute_outer(self, dt):
        ...
        # ==============================================================
        # HYBRID STEERING = BLEND(cascade_PD, backstepping)
        # ==============================================================

        # --- Cascade PD path ---
        p_lat = (
            self.pfloat("outer_kp_lat") * e_lat
        )
        d_lat = (
            self.pfloat("outer_kd_lat") * self.de_lat_f
        )
        p_heading = (
            self.pfloat("outer_kp_heading") * e_heading
        )
        d_heading = (
            self.pfloat("outer_kd_heading") * self.de_heading_f
        )
        omega_pd = (
            self.pfloat("outer_control_sign")
            * gain_multiplier
            * (p_lat + d_lat + p_heading + d_heading)
        )

        # --- Backstepping path ---
        lambda_y = max(
            self.pfloat("lambda_y"), 0.001
        )
        theta_virtual = math.atan(
            lambda_y * e_lat
        )
        # virtual heading derivative:
        # d(theta_v)/dt = lambda_y/(1+(lambda_y*e_lat)^2) * de_lat
        dtheta_v = (
            lambda_y
            / (1.0 + (lambda_y * e_lat) ** 2)
            * self.de_lat_f
        )
        e_bs      = e_heading + theta_virtual
        de_bs     = self.de_heading_f + dtheta_v

        omega_bs = (
            self.pfloat("outer_control_sign")
            * gain_multiplier
            * (
                self.pfloat("k_y")       * e_lat
                + self.pfloat("k_dy_bs") * self.de_lat_f
                + self.pfloat("k_theta") * e_bs
                + self.pfloat("k_dtheta_bs") * de_bs
            )
        )

        # --- Blend ---
        w = clamp(self.pfloat("bs_weight"), 0.0, 1.0)
        omega_feedback = (
            (1.0 - w) * omega_pd
            + w       * omega_bs
        )
