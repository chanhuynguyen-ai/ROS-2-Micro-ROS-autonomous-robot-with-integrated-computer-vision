...
def analyze(lines):
    ...
    events = []
    for l in lines:
        is_high_error = abs(l.get('epsilon_x_mm', 0)) > 60 or abs(l.get('theta_rad', 0)) > 0.15
        is_curve = 'curve' in l.get('mode', '')
        is_large = 'large' in l.get('mode', '')
    ...
    for i, ev in enumerate(events[:5]):
        ...
        for j, e in enumerate(ev):
            if j % 5 == 0 or j == len(ev)-1:
                print(f"  [{j:3d}] mode={e.get('mode')[:15]:15s} e_x={e.get('epsilon_x_mm',0):6.1f} th={e.get('theta_rad',0):6.3f} v={e.get('v_cmd',0):.3f} om={e.get('omega_cmd',0):.3f} om_lim={e.get('omega_limit',0):.3f} dV={e.get('delta_v_cmd',0):.3f}")
